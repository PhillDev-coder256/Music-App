...
import 'provider/album_details_provider.dart';

class AlbumDetailsScreen extends StatefulWidget {
  const AlbumDetailsScreen({Key? key}) : super(key: key);

  @override
  AlbumDetailsScreenState createState() => AlbumDetailsScreenState();
...
class AlbumDetailsScreenState extends State<AlbumDetailsScreen> {
  ...
  Widget _buildSongsList(BuildContext context) {
    return Consumer<AlbumDetailsProvider>(builder: (context, provider, child) {
      return ListView.separated(
          ...
          itemCount: provider.albumDetailsModelObj.songslistItemList.length,
          itemBuilder: (context, index) {
            SongslistItemModel model =
                provider.albumDetailsModelObj.songslistItemList[index];
